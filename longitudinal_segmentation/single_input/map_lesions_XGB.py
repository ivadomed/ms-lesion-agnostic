...
Inputs:
    ...
    - inference_csv: Path to the csv dataset containing lesion features for the inference.
...
def main():
    ...
    # Transform the df to create one row per lesion pair in timepoint1 and timepoint2 (pairing only within the same subject)
    ## Columns are : subject,timepoint,group,z,r,theta,volume
    lesion_pairs_inf = []
    subjects_ing = df_inference['subject'].unique()

    for subject in subjects_ing:
        df_subject = df_inference[df_inference['subject'] == subject]
        timepoints = df_subject['timepoint'].unique()
        if len(timepoints) != 2:
            continue  # We only process subjects with exactly 2 timepoints
        tp1, tp2 = timepoints
        ...
        for _, lesion1 in df_tp1.iterrows():
            for _, lesion2 in df_tp2.iterrows():
                ...
                # Add label if available (1 if they are the same lesion, else 0)
                pair_features['label'] = 1 if lesion1['group'] == lesion2['group'] else 0
                lesion_pairs_inf.append(pair_features)

    # Create a DataFrame from lesion pairs
    df_pairs_inf = pd.DataFrame(lesion_pairs_inf)

    print(df_pairs_inf.head())

    print(df_pairs_inf.columns)

    # Perform inference
    X_inf = df_pairs_inf[feature_cols]
    y_inf_pred = model.predict(X_inf)
    df_pairs_inf['predicted_label'] = y_inf_pred

    # Save the inference in lesion mapping json files per subject
    subjects_inf = df_pairs_inf['subject'].unique()
    for subject in subjects_inf:
        df_subject = df_pairs_inf[df_pairs_inf['subject'] == subject]
        lesion_mapping = {}
        lesion_timepoint_1_ids = df_subject['lesion_id1'].unique()
        for lesion_id_1 in lesion_timepoint_1_ids:
            lesion_mappings = df_subject[(df_subject['lesion_id1'] == lesion_id_1) & (df_subject['predicted_label'] == 1)]
            lesion_timepoint_2_ids = lesion_mappings['lesion_id2'].tolist()
