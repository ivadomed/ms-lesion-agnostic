...
This script references both sc-seg and lesion-seg.
The datasets are:
...
for file in tqdm.tqdm(files):
    if 'SHA256' not in str(file) and file.name.replace('_PSIR.nii.gz','').replace('_STIR.nii.gz','') not in exclude_list:
        ...
        sc_mask = canproco_path / 'derivatives' / 'labels' / relative_path / file.name.replace('.nii.gz', '_seg-manual.nii.gz')
        # if the masks exists we add it either to training or testing
...
for file in tqdm.tqdm(files):
    if 'SHA256' not in str(file):
        ...
        # Get the SC seg file
        sc_mask = basel_path / 'derivatives' / 'labels' / relative_path / file.name.replace('.nii.gz', '_label-SC_seg.nii.gz')
...
count_sct_testing_inf = 0

lesion_files = list(sct_testing_path.rglob('*_lesion-manual.nii.gz'))
for lesion_mask in tqdm.tqdm(lesion_files):
    if 'SHA256' not in str(file):
        # we build the corresponding image
        file = str(lesion_mask).replace('_lesion-manual.nii.gz', '.nii.gz').replace('derivatives/labels/', '')
        #we find the sc seg
        sc_mask = lesion_mask.parent / lesion_mask.name.replace('_lesion-manual.nii.gz', '_seg-manual.nii.gz')
        # we add it to the conversion dictionary of training or testing depending on the ratio
        if sc_mask.exists():
            if rd.random() < train_ratio:
                conversion_dict_training[str(file)] = {'lesion':str(lesion_mask),'sc':str(sc_mask)}
...
print(f'SCT-Testing: {count_sct_testing_inf} images for inference')
...
for file in tqdm.tqdm(files):
    if 'SHA256' not in str(file):
        ...
        # find the corresponding sc seg
        sc_mask = lesion_mask.parent / lesion_mask.name.replace('lesions-manual', 'seg-manual')    
        # if the mask exists we add it either to training or testing
        if lesion_mask.exists() and sc_mask.exists():
            # we add it to the conversion dictionary of training or testing depending on the ratio
            if rd.random() < train_ratio:
