"""
This script is used to evaluate the lesion segmentation predictions. It is specific for the region-based model.
...
Example:
    python evaluate_lesion_seg_prediction.py --pred-folder path/to/predictions --mask-gt path/to/dataset --animaPath path/to/animaSegPerfAnalyzer --output-folder path/to/output_folder
...
def get_parser():
    ...
    parser.add_argument('--mask-gt', '-d', type=str, required=True, help='BIDS format containing the mask GT')
...
def analyze_prediction(prediction, lesion_mask, animaPath, output_folder):
    ...
    name = pathlib.Path(prediction).name.replace('.nii.gz','')
    ...
    print(f'Lesion mask: {lesion_mask}')
    print("-------------------")
    result_output_path = os.path.join(output_folder, 'tmp',name)
    os.system( f'{animaPath}/animaSegPerfAnalyzer -r {lesion_mask} -i {prediction} -o {result_output_path} -d -s -l -X -S')
    print('Done!')

    #we read the xml file (such as this : singleClassNnunetMsLesion_278_metrics_global.xml)
    xml_file = result_output_path + '_global.xml'
    # check if file exists : if it doesn't exist we skip the subject
    if not os.path.isfile(xml_file):
        print(f'Error: {xml_file} does not exist. Skipping subject {name}')
        return None
...
def main():
    ...
    if not os.path.isdir(os.path.join(output_folder, 'tmp','tmp_lesion_bin')):
        os.makedirs(os.path.join(output_folder, 'tmp','tmp_lesion_bin'))
    if not os.path.isdir(os.path.join(output_folder, 'tmp','tmp_gt_bin')):
        os.makedirs(os.path.join(output_folder, 'tmp','tmp_gt_bin'))

    #we initialize the results dataframe
    results_dataframe = pd.DataFrame()

    #we get the list of predictions 
    predictions = list(pathlib.Path(pred_folder).rglob(f'*.nii.gz'))

    #we get the list of the lesion masks
    lesion_masks = list(pathlib.Path(mask_gt).rglob(f'*.nii.gz'))

    #we iterate over the predictions
    for prediction in predictions:
        
        #we get the name of the corresponding subject
        pred_name = pathlib.Path(prediction).name

        # fing the corresponding lesion mask
        lesion_mask = [mask for mask in lesion_masks if pred_name in str(mask)][0]

        # we extract only the lesions and remove the spinal cord in the prediction file
        pred_mask = nib.load(str(prediction))
        pred_affine = pred_mask.affine
        pred_header = pred_mask.header
        pred_mask = np.asarray(pred_mask.dataobj)
        pred_mask = np.where(pred_mask > 1, 1, 0)
        # the temp output path of the file
        temp_pred_lesion_file = os.path.join(output_folder, 'tmp','tmp_lesion_bin',pred_name)
        pred_lesion_file = nib.Nifti1Image(pred_mask, pred_affine, pred_header)
        nib.save(pred_lesion_file, str(temp_pred_lesion_file))

        # we also binarise the GT in case they have more than 1 as values
        gt_mask = nib.load(str(lesion_mask))
        gt_affine = gt_mask.affine
        gt_header = gt_mask.header
        gt_mask = np.asarray(gt_mask.dataobj)
        gt_mask = np.where(gt_mask > 0.5, 1, 0)
        # the temp output path of the file
        gt_name = pathlib.Path(lesion_mask).name
        temp_gt_file = os.path.join(output_folder, 'tmp','tmp_gt_bin',gt_name )
        gt_file_bin = nib.Nifti1Image(gt_mask, gt_affine, gt_header)
        nib.save(gt_file_bin, str(temp_gt_file))

        #we analyze the prediction
        subject_results = analyze_prediction(temp_pred_lesion_file, temp_gt_file, animaPath, output_folder)
